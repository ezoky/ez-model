...
class DefineAnInteractionSpec
  ...
  "the StateProcessor" when {
    "the Modeller defines an Interaction" should {
      "sets it as the current Interaction" in {
        ...
        val whatISay =
          Say {
            theInteraction ("choose" the "Invoiced Customers")
          }

        Then("the modelling state changes accordingly")
        val definedInteraction =
          Interaction(Action(Verb("choose")), Some(ActionObject(NameGroup(Determinant.the, Name("Invoiced Customers")))))
